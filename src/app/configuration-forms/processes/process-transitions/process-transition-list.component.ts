...
import { ProcesoDTO, ProcesoTransicionDTO, ProcesoTransicionFilterDTO } from 'app/modules/full/neuron/model/sw42.domain';
...
@Component({
        ...
        <div class="flex-1 overflow-y-auto">
          <div class="overflow-x-auto">
            <table mat-table [dataSource]="transitions()" class="w-full">
              ...
              <ng-container matColumnDef="estadoPartida"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Estado Origen</th><td mat-cell class="px-4 py-3">{{ element.estadoPartidaNombre }}</td></ng-container>
              <ng-container matColumnDef="estadoLlegada"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Estado Destino</th><td mat-cell class="px-4 py-3">{{ element.estadoLlegadaNombre }}</td></ng-container>
              <ng-container matColumnDef="plantilla"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Plantilla</th><td mat-cell class="px-4 py-3">{{ element.plantillaNombre }}</td></ng-container>
              <ng-container matColumnDef="documentador"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Documentador</th><td mat-cell class="px-4 py-3 text-center"><span class="badge" [class.badge-success]="element.documentador" [class.badge-secondary]="!element.documentador">{{ element.documentador ? 'Sí' : 'No' }}</span></td></ng-container>
              <ng-container matColumnDef="afectaSaldo"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Afecta Saldo</th><td mat-cell class="px-4 py-3">{{ element.afectaSaldo }}</td></ng-container>
              <ng-container matColumnDef="rapida"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Rápida</th><td mat-cell class="px-4 py-3 text-center"><span class="badge" [class.badge-info]="element.rapida" [class.badge-secondary]="!element.rapida">{{ element.rapida ? 'Sí' : 'No' }}</span></td></ng-container>
              ...
              <ng-container matColumnDef="acciones"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Acciones</th><td mat-cell class="px-4 py-3"><div class="flex items-center justify-end gap-1"><button type="button" class="btn-icon btn-flat-primary" (click)="openTransitionForm(element)" aria-label="Editar"><mat-icon>edit</mat-icon></button><button type="button" class="btn-icon btn-flat-accent" (click)="toggleStatus(element)" aria-label="{{ element.estado === 'A' ? 'Inactivar' : 'Activar' }}"><mat-icon>{{ element.estado === 'A' ? 'block' : 'check_circle' }}</mat-icon></button></div></td></ng-container>
  ...
  `,
    ...
    :host ::ng-deep .mat-column-documentador, :host ::ng-deep .mat-column-rapida { width: 100px; text-align: center; }
  `]
})
