...
@Component({
    ...
    <div class="p-4 sm:p-6 space-y-4">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-200 dark:border-gray-700">
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
          ...
          <mat-form-field appearance="outline" class="w-full"><mat-label>Tipo</mat-label><mat-select [(ngModel)]="filter.tipo" (ngModelChange)="onFilterChange()"><mat-option value="">Todos</mat-option><mat-option value="DB">Base de Datos</mat-option><mat-option value="APP">Aplicación</mat-option><mat-option value="WEB">Web</mat-option><mat-option value="MAIL">Correo</mat-option></mat-select></mat-form-field>
          ...
          <mat-form-field appearance="outline" class="w-full"><mat-label>Activo</mat-label><mat-select [(ngModel)]="filter.activo" (ngModelChange)="onFilterChange()"><mat-option value="">Todos</mat-option><mat-option [value]="true">Sí</mat-option><mat-option [value]="false">No</mat-option></mat-select></mat-form-field>
        </div>
      </div>
        ...
        @if (loading()) { <div class="flex justify-center py-12"><mat-spinner diameter="40"></mat-spinner></div> } @else {
          <div class="overflow-x-auto">
            <table mat-table [dataSource]="data()" class="w-full">
              ...
              <ng-container matColumnDef="tipo"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Tipo</th><td mat-cell class="px-4 py-3"><span class="badge" [class]="getTipoBadge(element.tipo)">{{ element.tipo }}</span></td></ng-container>
              <ng-container matColumnDef="url"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">URL</th><td mat-cell class="px-4 py-3 font-mono text-sm">{{ element.url }}</td></ng-container>
              <ng-container matColumnDef="baseDatos"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Base Datos</th><td mat-cell class="px-4 py-3">{{ element.baseDatos }}</td></ng-container>
              <ng-container matColumnDef="puerto"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Puerto</th><td mat-cell class="px-4 py-3">{{ element.puerto }}</td></ng-container>
              <ng-container matColumnDef="activo"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Activo</th><td mat-cell class="px-4 py-3"><span class="badge" [class.badge-success]="element.activo" [class.badge-secondary]="!element.activo">{{ element.activo ? 'Sí' : 'No' }}</span></td></ng-container>
  ...
  `,
    ...
    .badge-tipo-db { background: #e3f2fd; color: #1565c0; }
    .badge-tipo-app { background: #e8f5e9; color: #2e7d32; }
    .badge-tipo-web { background: #f3e5f5; color: #6a1b9a; }
    .badge-tipo-mail { background: #fff3e0; color: #ef6c00; }
...
export class ServerListComponent implements OnInit {
    ...
    filter = signal<ServidorFilterDTO>({ estado: 'A', nombre: '', tipo: '', activo: undefined, paginacionRegistroInicial: 0, paginacionRegistroFinal: 25 });
    displayedColumns = ['nombre', 'tipo', 'url', 'baseDatos', 'puerto', 'activo', 'estado', 'acciones'];
    ...
    getTipoBadge(tipo: string): string {
        const badges: Record<string, string> = { 'DB': 'badge-tipo-db', 'APP': 'badge-tipo-app', 'WEB': 'badge-tipo-web', 'MAIL': 'badge-tipo-mail' };
