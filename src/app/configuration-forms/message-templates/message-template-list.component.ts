...
@Component({
        ...
        @if (loading()) { <div class="flex justify-center py-12"><mat-spinner diameter="40"></mat-spinner></div> } @else {
          <div class="overflow-x-auto">
            <table mat-table [dataSource]="data()" class="w-full">
              ...
              <ng-container matColumnDef="asunto"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Asunto</th><td mat-cell class="px-4 py-3 truncate max-w-xs">{{ element.asunto }}</td></ng-container>
              <ng-container matColumnDef="cuerpo"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Cuerpo (resumen)</th><td mat-cell class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300 truncate max-w-md">{{ element.cuerpo }}</td></ng-container>
              <ng-container matColumnDef="adjuntos"><th mat-header-cell class="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Adjuntos</th><td mat-cell class="px-4 py-3">{{ element.adjuntos || 'Ninguno' }}</td></ng-container>
  ...
  `,
    ...
    .badge-tipo-email { background: #e3f2fd; color: #1565c0; }
    .badge-tipo-sms { background: #e8f5e9; color: #2e7d32; }
    .badge-tipo-push { background: #f3e5f5; color: #6a1b9a; }
...
export class MessageTemplateListComponent implements OnInit {
    ...
    filter = signal<MensajePlantillaCorreoFilterDTO>({ estado: 'A', nombre: '', tipo: '', paginacionRegistroInicial: 0, paginacionRegistroFinal: 25 });
