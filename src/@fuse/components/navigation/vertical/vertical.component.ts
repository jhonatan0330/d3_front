...
export class FuseVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy {
    ...
    readonly fuseScrollbarDirectives = viewChildren(FuseScrollbarDirective);
    ...
    constructor() {
        ...
        effect(() => {
            const fuseScrollbarDirectives = this.fuseScrollbarDirectives();
    ...
    onWindowScroll(): void {
        const navigationElement = document.querySelector('.fuse-vertical-navigation-wrapper') as HTMLElement;
        if (navigationElement) {
            navigationElement.style.top = '0'; // Aquí aseguramos que siempre se quede en el top
        }
    }
